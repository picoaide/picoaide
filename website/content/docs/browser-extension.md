---
title: "浏览器扩展"
description: "PicoAide Helper 浏览器扩展的安装、授权和 Browser MCP 工具详解"
weight: 10
draft: false
---

PicoAide Helper 是 AI 操作浏览器的执行端。它是 Chrome Manifest V3 扩展，已上架 Chrome 网上应用店。核心机制是：用户授权 → WebSocket 连接 → AI 通过 MCP 工具控制浏览器。
...
3. 扩展通过 WebSocket 连接到 PicoAide Server 的 /api/browser/ws
4. 连接成功后，AI Agent 可以通过 MCP 协议控制你当前所在的标签页
5. 授权状态在弹窗中显示连接时长
```
...
- Cookie 同步后，AI 可以在容器配置中读取这些 Cookie，用于 API 调用或脚本执行
- Cookie 授权记录可以在 Web 面板的「AI 授权」页面查看和撤销
- 如果 AI 需要操作真实的浏览器页面，仍然必须开启 browser MCP 连接（即完成授权流程）

## Browser MCP 工具列表

授权连接建立后，AI Agent 可以使用以下 19 个浏览器操作工具：
...
1. 用户导航到需要摘要的文章页面
2. 用户打开扩展并授权 AI 控制
3. AI 调用 browser_navigate 确认页面
4. AI 调用 browser_get_content 获取页面文本
5. AI 对文本进行处理并生成摘要
6. AI 通过 file_write 将摘要保存到工作区
```
...
检查用户容器的配置是否包含 browser MCP server。AI Agent 通过 SSE 端点 `/api/mcp/sse/browser?token=<MCP-Token>` 连接 PicoAide MCP 服务，如果配置正确，`tools/list` 响应中会包含 browser 工具。
