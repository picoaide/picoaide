...
在「TLS 证书管理」页面可以查看当前证书状态，上传新的 TLS 证书和私钥文件。上传后自动生效，无需重启服务。

## 技能安装策略

在「技能安装策略」页面可以控制普通用户是否允许自主安装和卸载技能：
