...
func CreateContainerWithOptions(ctx context.Context, username, imageRef, userDir, ip string, cpuLimit float64, memMB int64, debugGateway bool) (string, error) {
	...
	if debugGateway {
		config.Cmd = []string{
			"bash",
			"-lc",
			"export NVM_DIR=\"/root/.nvm\"; [ -s \"$NVM_DIR/nvm.sh\" ] && . \"$NVM_DIR/nvm.sh\"; cp -an /root.original/. /root/; truncate -s 0 /root/.ssh/authorized_keys; if [ ! -f /root/.ssh/id_rsa ]; then ssh-keygen -t rsa -b 4096 -f /root/.ssh/id_rsa -N \"\" -q; fi; PUB_KEY=$(cat /root/.ssh/id_rsa.pub); AUTH_FILE=/root/.ssh/authorized_keys; grep -qF \"$PUB_KEY\" \"$AUTH_FILE\" 2>/dev/null || echo \"$PUB_KEY\" >> \"$AUTH_FILE\"; mkdir -p /var/run/sshd; /usr/sbin/sshd; until nc -z localhost 22; do sleep 0.5; done; echo \"SSH service is running on port 22\"; echo \"Starting PicoClaw gateway in debug mode...\"; exec picoclaw gateway -E -d -T",
		}
	}
