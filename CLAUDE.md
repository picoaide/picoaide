...
internal/auth/         SQLite 数据库：本地超管、用户快照、组、容器状态记录 + IP 分配
internal/authsource/   统一认证源抽象、注册表、LDAP/OIDC 等 provider 实现和同步编排
internal/ldap/         LDAP 底层客户端：查询用户、认证、组查询（由 authsource/ldap.go 封装）
...
- **配置合并策略**：SQLite settings 表中的全局配置通过 `util.MergeMap()` 合并到各用户配置中——用户已有的键值保留，缺失的键从全局默认值补充。
- **Web 认证**：本地认证只保留超管和应急入口；普通用户来自当前启用的统一认证源（如 LDAP/OIDC）。会话使用 HMAC 签名的 Cookie（非 JWT）。CSRF 令牌使用按小时滚动的时间窗口。
- **JSON API**：所有 Web 端点返回 JSON。API 专为浏览器扩展消费而设计。

### 认证源扩展规范

统一认证源必须通过 `internal/authsource` 扩展，禁止把新认证源逻辑继续堆到 `internal/web`、`internal/auth` 或单个大文件里。

- `internal/authsource/provider.go` 只放认证源接口和注册表：
  - `PasswordProvider`：用户名密码认证，例如 LDAP。
  - `BrowserProvider`：浏览器跳转/回调认证，例如 OIDC、SAML。
  - `DirectoryProvider`：可枚举用户、组和成员关系的目录源。
  - `Register(name, provider)`：注册认证源实现。
- 每一种认证源独立一个文件，例如：
  - `internal/authsource/ldap.go`
  - `internal/authsource/oidc.go`
  - 后续新增 `saml.go`、`feishu.go`、`wecom.go`、`oauth2.go` 等。
...
- 切换 `web.auth_mode` 时必须走统一清理逻辑，清空旧普通用户、组成员、容器记录、用户目录和归档目录，防止不同认证源之间数据污染。
- 如果 provider 能提供组成员，使用 `auth.ReplaceGroupMembersBySource(source, groupMembers)` 替换对应来源的成员关系；不要写 LDAP 专属 SQL。
- 新增认证源必须至少补充：
  - provider 注册/能力断言单元测试；
  - claims 或用户映射测试；
  - 登录/同步相关集成测试（如该 provider 支持对应能力）。
...
- `picoaide.db`：SQLite 数据库，存储全局配置、白名单、用户账户和容器记录。
